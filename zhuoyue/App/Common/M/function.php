...
function getCrowdlist($parm=array()){
    $map= $parm['map'];
    $orderby= $parm['orderby'];
    ...
    if($parm['pagesize']){
        //分页处理
        import("ORG.Util.Page");
        $count = M('crowd_info ci')
            ->where($map)->count('ci.id');
        $p = new Page($count, $parm['pagesize']);
        $page = $p->show();
        $Lsql = "{$p->firstRow},{$p->listRows}";
    ...
    $pre = C('DB_PREFIX');
    $suffix=C("URL_HTML_SUFFIX");
    $list = M('crowd_info ci')->where($map)->order($orderby)->limit($Lsql)->select();
    $imgarray = array();
...
function crowd_status($crowd_id,$status = 1)
{
    switch($status){
        case 7:
            $href = '<a class="tz_bt_cr">我要预约</a>';
            break;
        case 1:
            $href = '<a class="tz_bt" href="'.U('M/Crowdinvest/detail', array('id'=>$crowd_id)).'">我要认筹</a>';
            break;
        case 2:
            $href = '<a class="tz_bt" href="'.U('M/Crowdinvest/detail', array('id'=>$crowd_id)).'">出售中</a>';
            break;
        case 3:
            $href = '<a class="tz_bt" href="'.U('M/Crowdinvest/detail', array('id'=>$crowd_id)).'">投票中</a>';
            break;
        case 4:
            $href = '<a class="tz_bt" href="'.U('M/Crowdinvest/detail', array('id'=>$crowd_id)).'">待回款</a>';
            break;
        case 5:
            $href = '<a class="tz_btt" >已售出</a>';
            break;
        case 8:
            $href = '<a class="tz_btt" >溢价回购</a>';
            break;
    }
    return $href;
}
...
{   
    ...
    if(empty($tqfee[2])){
        $one_limit = $fee[1][0]*10000;
        $today_limit = $fee[1][1]/$fee[1][0]; 
        $today_all_money = $fee[1][1];
    }else{
        $one_limit = $fee[2][0]*10000;
        $today_limit = $fee[2][1]/$fee[2][0];
        $today_all_money = $fee[2][1];
    }  
    if($money < 100 || $money > $one_limit) return "单笔提现金额限制为100-{$one_limit}元";
    if($today_time>=$today_limit){
                return "一天最多只能提现{$today_limit}次";
    }
        if(($today_money+$money)>$today_all_money*10000){
            return  "单日提现上限为{$today_all_money}万元。您今日已经申请提现金额：{$today_money}元,当前申请金额为:{$money}元,已超出单日上限，请您修改申请金额或改日再申请提现";
        }
