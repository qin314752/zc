    ...
    <div class="list">
        <table id="area_list" width="100%" border="0" cellspacing="0" cellpadding="0">
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr overstyle='on' id="list_<?php echo ($vo["id"]); ?>">
                    ...
                    <td>
                        <?php if($vo["status"] == 2 && $vo["voting_status"] == ''): ?><a  href="__URL__/settings?id=<?php echo ($vo["id"]); ?>">发起投票</a>
                        <?php else: ?>
                            <?php if($vo["voting_status"] == 1): ?>投票中
                                <?php elseif($vo["voting_status"] == 3): ?>
                                投票成功
                                <?php else: ?>
                                <span style="color: red">投票失败&nbsp;&nbsp;&nbsp;</span>
                                <a  href="__URL__/settings?id=<?php echo ($vo["id"]); ?>">再次投票</a><?php endif; endif; ?>

                    </td>
                    <td>
                        <a  href="__URL__/fullylist?id=<?php echo ($vo["voting_id"]); ?>">查看</a>
                    </td>
