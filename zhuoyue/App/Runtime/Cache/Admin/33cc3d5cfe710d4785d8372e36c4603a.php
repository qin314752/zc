...
<div class="so_main">
    ...
    <div class="form2">
        <form method="post" action="__URL__/doedit" onsubmit="return subcheck();" enctype="multipart/form-data">
            <div id="tab_1">
                <input type="hidden" value="<?php echo ($crowd_info["id"]); ?>" name="id"/>
                <dl class="lineD">
                    <dt>众筹项目名称：</dt>
                    <dd>
                        <input name="crowd_name" id="crowd_name"  class="input" type="text" value="<?php echo ($crowd_info["crowd_name"]); ?>" ><span id="tip_crowd_name" class="tip">*</span>
                    </dd>
                </dl>
                <dl class="lineD">
                    <dt>发起人：</dt>
                    <dd>
                        <select name="user_name" id="user_name"   class="c_select"><option value="">--请选择--</option><?php foreach($user_name as $key=>$v){ if($crowd_info["user_id"]==$key && $crowd_info["user_id"]!=""){ ?><option value="<?php echo ($key); ?>" selected="selected"><?php echo ($v); ?></option><?php }else{ ?><option value="<?php echo ($key); ?>"><?php echo ($v); ?></option><?php }} ?></select><span id="tip_user_name" class="tip">*</span>
                    </dd>
                </dl>
                <dl class="lineD">
                    <dt>项目金额：</dt>
                    <dd>
                        <input style="width: 353px;height: 17px;" type="text" value="<?php echo ($crowd_info["crowd_money"]); ?>" id="crowd_money" name="crowd_money" onkeyup="value=changestr(value)" /></td><td>*</td>
                    </dd>
                </dl>
                <dl class="lineD">
                    <dt>上线：</dt>
                    <dd>
                        <?php $i=0;$___KEY=array ( 0 => '无效', 1 => '有效', ); foreach($___KEY as $k=>$v){ if(strlen("1key")==1 && $i==0){ ?><input type="radio" name="is_use" value="<?php echo ($k); ?>" id="is_use_<?php echo ($i); ?>" checked="checked" /><?php }elseif(("key1"=="key1"&&$crowd_info["is_use"]==$k)||("key"=="value"&&$crowd_info["is_use"]==$v)){ ?><input type="radio" name="is_use" value="<?php echo ($k); ?>" id="is_use_<?php echo ($i); ?>" checked="checked" /><?php }else{ ?><input type="radio" name="is_use" value="<?php echo ($k); ?>" id="is_use_<?php echo ($i); ?>" /><?php } ?><label for="is_use_<?php echo ($i); ?>"><?php echo ($v); ?></label>&nbsp;&nbsp;&nbsp;&nbsp;<?php $i++; } ?>
                    </dd>
                </dl>
                <dl class="lineD">
                    <dt>开始时间：</dt>
                ...
                <dl class="lineD">
                    <dt>集资期限：</dt>
                    <dd>
                        <select name="zc_collect" id="zc_collect"   class="c_select"><option value="">--请选择--</option><?php foreach($zc_collect as $key=>$v){ if($crowd_info["crowd_duration"]==$key && $crowd_info["crowd_duration"]!=""){ ?><option value="<?php echo ($key); ?>" selected="selected"><?php echo ($v); ?></option><?php }else{ ?><option value="<?php echo ($key); ?>"><?php echo ($v); ?></option><?php }} ?></select><span id="tip_zc_collect" class="tip">*(天)</span>
                ...
                <dl class="lineD">
                    <dt>众筹最小金额：</dt>
                    <dd>
                        <select name="zc_min" id="zc_min"   class="c_select"><option value="">--请选择--</option><?php foreach($zc_min as $key=>$v){ if($crowd_info["min_money"]==$key && $crowd_info["min_money"]!=""){ ?><option value="<?php echo ($key); ?>" selected="selected"><?php echo ($v); ?></option><?php }else{ ?><option value="<?php echo ($key); ?>"><?php echo ($v); ?></option><?php }} ?></select><span id="tip_zc_min" class="tip">*(元)</span>
                    </dd>
                </dl>
                ...
                <dl class="lineD">
                    <dt>最长持有期限：</dt>
                    <dd>
                        <select name="zc_deadline" id="zc_deadline"   class="c_select"><option value="">--请选择--</option><?php foreach($zc_deadline as $key=>$v){ if($crowd_info["max_duration"]==$key && $crowd_info["max_duration"]!=""){ ?><option value="<?php echo ($key); ?>" selected="selected"><?php echo ($v); ?></option><?php }else{ ?><option value="<?php echo ($key); ?>"><?php echo ($v); ?></option><?php }} ?></select><span id="tip_zc_deadline" class="tip">*(月)</span>
                    </dd>
                </dl>
                <dl class="lineD">
                    <dt>分红比例：</dt>
...
function getEditorWordCount() {
	...
	return count;
                        ...
                        <!-- 编辑器调用开始 -->
				<textarea name="crowd_info" id="crowd_info" style="width:780px;height:320px;"><?php echo ($crowd_info["car_parameter"]); ?></textarea>
				<script>
                ...
                <dl class="lineD">
                    ...
                    <dd>
                        <!-- 编辑器调用开始 -->
				<textarea name="crowd_car_zk" id="crowd_car_zk" style="width:780px;height:320px;"><?php echo ($crowd_info["car_info"]); ?></textarea>
				<script>
                                ...
                                <td colspan="4" class="bline" style="background:url(images/albviewbg.gif) #fff 0 20px no-repeat;"><table width='100%' height='160' style="margin:0 0 20px 0">
                                    <tr>
                                        <td>
                                            ...
                                            <div id="thumbnails">
                                                <?php $x=1000;foreach(unserialize($crowd_info['crowd_picture']) as $v){ $x--; ?>
                ...
                <dl class="lineD">
                    ...
                    <dd>
                        <!-- 编辑器调用开始 -->
				<textarea name="crowd_agreement" id="crowd_agreement" style="width:780px;height:320px;"><?php echo ($crowd_info["crowd_agreement"]); ?></textarea>
				<script>
