...
class CommonAction extends ACommonAction
{
    ...
    public function sendsms(){
    	...
    	if(intval($_POST['sms'])==1){//账户通讯
    		...
    		$user = M('members m')->field(" m.id,m.user_email,m.user_phone,ms.email_status,ms.phone_status ")->join(" nuomi_members_status ms ON m.id=ms.uid ")->where(" m.user_name = '".$user_name."' ")->find();
    ...
    public function sendgala(){
        ...
        $user = M('members m')->field(" m.id,m.user_email,m.user_phone,ms.email_status,ms.phone_status ")->join(" nuomi_members_status ms ON m.id=ms.uid ")->where($map)->select();
