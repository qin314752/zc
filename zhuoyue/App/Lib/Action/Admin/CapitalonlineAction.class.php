...
//		if(session('admin_is_kf')==1)	$map['m.customer_id'] = session('admin_id');
     ...
     public function withdraw()
    {
		...
		$list = M('member_withdraw w')->field($field)->join("{$this->pre}members m ON w.uid=m.id")->join("nuomi_member_money mm on w.uid = mm.uid")->where($map)->limit($Lsql)->order("w.id DESC")->select();
