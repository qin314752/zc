...
class MarketAction extends ACommonAction
{
    public function index(){
		...
		if($status_list){
			...
			if($map['l.add_time'])	$status_html .= '自<font style="color:#fc8936;">'.$_REQUEST['start_time'].'</font>至<font style="color:#fc8936;">'.$_REQUEST['end_time'].'</font> ';

			for ($i=0; $i < 5; $i++) { 
				if($status_list[$i])	$status_html .= $type_list[$i].':<font style="color:#fc8936;">'.$status_list[$i].'</font>分 ';
			}
			$status_html .= '</div>';
	...
	public function getlog(){
		...
		if($ty_list){
			...
			for ($i=0; $i < 4; $i++) { 
				if($ty_list[$i])	$status_html .= $type_list[$i].'<font style="color:#fc8936;">'.$ty_list[$i]['num'].'</font>次 总价值<font style="color:#fc8936;">'.$ty_list[$i]['money'].'</font>元 ';
			}
		}
		$money = M('market_log l')->join("{$this->pre}members m ON m.id=l.uid")->where($map)->sum("l.price");
		$status_html .= '当前选中<font style="color:#fc8936;">'.$count.'</font>项 总价值<font style="color:#fc8936;">'.$money.'</font>元 ';
		$status_html .= '</div>';
		$this->assign('status_html',$status_html);
