    ...
    */
    public function index()
    {
		...
		//$count = M('members m')->join("{$this->pre}member_money mm ON mm.uid=m.id")->join("{$this->pre}member_info mi ON mi.uid=m.id")->join("{$this->pre}members_status ms ON ms.uid=m.id")->where($map)->count('m.id');
                $count = M('members_status ms')->join("{$this->pre}member_money mm ON ms.uid=mm.uid")->join("{$this->pre}member_info mi ON ms.uid=mi.uid")->join("{$this->pre}members m ON ms.uid=m.id")->where($map)->count('m.id');
		...
		$list = M('members_status ms')->field($field)->join("{$this->pre}member_money mm ON ms.uid=mm.uid")->join("{$this->pre}member_info mi ON ms.uid=mi.uid")->join("{$this->pre}members m ON ms.uid=m.id")->where($map)->limit($Lsql)->order('m.id DESC')->select();
