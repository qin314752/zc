...
class MCommonAction extends Action
{
	...
	protected function _initialize(){
        if(session("u_id")&&strtolower(ACTION_NAME)!="actlogout") {
            ...
            $pre = C('DB_PREFIX');
            $checkinfo = M("members m")->field("m.pin_pass,s.id_status")->join("{$pre}members_status s ON s.uid=m.id")->where("m.id={$this->uid}")->find();//var_dump($checkinfo);exit();
	    ...
	    $tel = M(qq)->where(array('type'=>2))->find();
		$this->assign('tel',$tel);
		...
		//qq
		  $qq = M(qq)->where(array('type'=>0))->find();
		  $this->assign('qq',$qq);
		  //qq群
		  $qun = M('qq')->where(array('type'=>1))->find();
		  $this->assign('qun',$qun);
        ...
        $map['is_hiden'] = 0;
        $map['parent_id'] = 8;
        $data = D('Acategory')->field($field)->where($map)->order($Osql)->limit(6)->select();
        //Á´½Ó´¦Àí
		...
		//left
        $leftlist = M('ArticleCategory')->where(array('type_set'=>1))->limit(5)->select();
