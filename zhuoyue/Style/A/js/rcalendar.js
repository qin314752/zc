...
function rcalendar(text, mode, retfunction) { //文本域对象, 模式(dateonly,full), 选择日期后的事件函数(函数是新时间的Date对象)
	...
	if (!div_rcalendar) { //如果不存在，则初始化创建它
		...
		if (document.compatMode == "BackCompat" && navigator.userAgent.indexOf("MSIE") != -1) {
			css += "#rcalendar {width:200px; height:200px; background:#FFFFFF; font-size:12px; font-family:微软雅黑; border:1px solid #999999; padding:4px; overflow:hidden;}";
			...
			css += "#rcalendar {width:190px; height:190px; background:#FFFFFF; font-size:12px; font-family:微软雅黑; border:1px solid #999999; padding:4px; overflow:hidden;}";
			css += "#rcalendar_ym {width:190px; height:14px; overflow:hidden; margin-bottom:4px;}";
