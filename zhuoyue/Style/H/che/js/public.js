...
$(document).ready(function() {
    ...
    //公共顶部鼠标放上去显示图片
    $(".ico-phone,.ico-wx,.wechat").hover(function() {
    ...
    //显示提示信息框
    $(".tips").hover(function() {
            $(this).find(".up,.tipsinfo").css("display", "block")
        },
        function() {
            $(this).children(".up,.tipsinfo").css("display", "none")
        })
    //导航鼠标放上去效果
    $('.header .menu li').hover(function() {
            $(this).stop().addClass("select");
            $('.header .menu span.s1').stop().animate({
                'left': ($(this).position().left + 20),
                'width': ($(this).width())
            });
        },
        function() {
            $(this).stop().removeClass("select");
            $('.header .menu span.s1').stop().animate({
                    'left': ($('.header .menu li.focus').position().left + 20),
                    'width': ($('.header .menu li.focus').width())
                },
                500);
        })
    $('.header .menu span.s1').stop().animate({
...
function updateEndTime() {
    $(".settime").each(function(i) {
        var endTime = parseInt(this.getAttribute("endTime")); //结束时间字符串
        var lag = endTime - j; //当前时间和结束时间之间的秒数
        if (lag > 0) {
            ...
            var cfId = ($("#my_param").attr("data-id") == 0) ? '': $("#my_param").attr('data-id');
            var obj = $(".settime").attr('data-value');
            if (cfId != '') {
                $(".ico-clock").css('visibility', 'hidden');
