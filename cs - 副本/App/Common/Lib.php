...
function Notice($type,$uid,$data=array()){
		...
		//import('ORG.Net.EmailShop');
		  import('ORG.Net.Mail');
		$smtp = new Email();
		$sender = $msgconfig['stmp']['user'];
		$sender_pass =  $msgconfig['stmp']['pass'];
		$sender_name = $msgconfig['stmp']['name'];
	...
	switch($type){
		...
		case 1://注册成功发送邮件
			...
			$to = $minfo['user_email']; 
			//$send=$smtp->send($to,$subject,$body);
			$send = $smtp->sendMail($body,$subject,$sender,$sender_pass,$sender_name,$to,$to);
			//将邮件发送记录存到数据库中	
